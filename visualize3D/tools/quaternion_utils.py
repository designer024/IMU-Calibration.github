...
def quat_to_pose(quat, bone_len, rigid_torso):
...
def quats_to_poses(quats, bone_len, rigid_torso):
    ...
    if len(quats_copy.shape) == 3:
        ...
        ret = np.empty((len(quats_copy),8,3))
        for i in range(len(quats_copy)):
            ret[i] = quat_to_pose(
                quats_copy[i], bone_len, rigid_torso
            )
        return ret
    else:
        # quats = (sequences, frames, joints, 4)
        k, n, _, _ = quats_copy.shape
        ret = np.empty((k,n,8,3))
        for j in range(k):
            for i in range(n):
                ret[j][i] = quat_to_pose(
                    quats_copy[j][i], bone_len, rigid_torso
                )
        return ret
